...
class ProductGenerator {
    ...
    updateFeaturedProducts() {
        const featuredDescriptions = {
            "skyfall.webp": "A bold and refreshing fragrance that captures the essence of adventure and sophistication. Features invigorating citrus top notes with a heart of aromatic woods and a deep, masculine base.",
            "canella.webp": "A warm and spicy fragrance centered around cinnamon and rich woods. Combines sweet and spicy notes with a creamy vanilla and sandalwood base for a comforting yet sophisticated scent.",
            "horus.webp": "An exotic and mysterious fragrance inspired by ancient Egypt. Blends incense, saffron, and warm spices with a rich amber and papyrus base.",
            "italian-spirit.webp": "A bright and vibrant fragrance capturing the essence of the Italian Mediterranean. Features zesty citrus, aromatic herbs, and sun-warmed fig with a mossy base.",
            "at-the-sunsets.webp": "A romantic and dreamy fragrance that evokes peaceful coastal evenings. Combines warm amber, sea salt, and coconut with vanilla and sandalwood for a sunset-inspired experience.",
            "classico.webp": "A timeless and elegant fragrance that honors traditional perfumery. Features bergamot, aromatic lavender, and precious woods with a sophisticated amber base."
        };

        document.querySelectorAll('.product-card.featured').forEach(card => {
            const imageSrc = card.querySelector('img').src.split('/').pop();
            if (featuredDescriptions[imageSrc]) {
                const descElement = document.createElement('div');
                descElement.classList.add('product-description');
                descElement.style.display = 'none';
                descElement.innerHTML = featuredDescriptions[imageSrc];
                card.appendChild(descElement);
                const shortDesc = document.createElement('p');
                shortDesc.classList.add('short-description');
                shortDesc.textContent = featuredDescriptions[imageSrc].substring(0, 60) + "...";
                card.insertBefore(shortDesc, card.querySelector('.add-to-cart'));
            }
        });
    }
}
